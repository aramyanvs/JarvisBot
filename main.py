...
SYSTEM_PROMPT="Ты Jarvis — русскоязычный ассистент. Отвечай по делу. Если нужна свежая информация, используй сводку, приложенную в system."
UA="Mozilla/5.0"
...
async def on_read(c:Client,m:Message):
    ...
    try: raw=await fetch_url(parts[1])
    except Exception as e: return await m.reply_text(f"Ошибка: {e}")
