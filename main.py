...
async def save_memory(uid:int, mem):
    ...
    try:
        await c.execute("""
            insert into users(user_id,memory) values($1,$2::jsonb)
...
def read_file(p):
    ext=p.lower()
    if ext.endswith((".txt",".md",".log")): return open(p,"r",encoding="utf-8",errors="ignore").read()
    if ext.endswith(".pdf"): return pdf_text(p) or ""
...
async def on_text(update:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    elif any(k in text.lower() for k in ["новост","цена","сейчас","погода","курс","расписан","релиз","обнов"]):
    ...
    try:
        ...
        await c.execute("""
            ...
            WHERE memory IS NULL
               OR memory::text=''
               OR NOT (
                    (pg_typeof(memory)::text='jsonb')
                 OR ((memory::text) IS JSON)
               )
        """)
        await c.execute("""
            ALTER TABLE users
